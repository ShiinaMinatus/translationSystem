...
class TranslationAction extends Action {
    ...
    function submitEnglishResume() {
        ...
        if ($resumeValues == "true") {
            $RetrunValue = "您的简历翻译已被提交，请耐心等待结果。<a href='" . WebSiteUrl . "user/managerPage'>返回</a>用户中心";
        } else {
    ...
    function undoneTranslation() {
        isLogin();
        $userId = $_SESSION["userId"];
        $data['userId'] = $userId;
        $reValue = transferData(API_URL . "resume/undoneResumeTranslation", "post", $data);
        $reValue = json_decode($reValue, true);
